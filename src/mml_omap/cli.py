...
def lidar_diagnostic_plot_geometry(transform: RenderTransform, dpi: int) -> dict[str, int | float]:
    px_per_mm = dpi / 25.4
...
def lidar_diagnostic_grid_shape(transform: RenderTransform) -> tuple[int, int]:
...
def lidar_diagnostic_cell_bounds(transform: RenderTransform, column: int, row: int) -> tuple[float, float, float, float]:
...
def paint_lidar_diagnostic_cell(
    image: Any,
    transform: RenderTransform,
    ...
    x0, y0, x1, y1 = lidar_diagnostic_cell_bounds(transform, column, row)
...
def draw_lidar_diagnostic_frame(image: Any, metrics: dict[str, int | float]) -> None:
...
def lidar_diagnostic_surfaces(source_data: dict[str, Any], transform: RenderTransform) -> dict[str, Any]:
    import numpy as np
    ...
    max_object_height = np.full((rows_count, columns), np.nan, dtype=float)
    ...
    if len(points):
        ...
        if len(object_heights):
            ...
            max_seed = np.where(np.isnan(max_object_height), -np.inf, max_object_height)
            np.maximum.at(max_seed, (object_rows, object_columns), object_heights)
            max_object_height[:] = np.where(np.isneginf(max_seed), np.nan, max_seed)
...
def hillshade_rgb(values: Any) -> Any:
    ...
    gray = np.rint(np.clip((shaded + 1.0) / 2.0, 0.0, 1.0) * 255).astype(np.uint8)
...
def render_lidar_raster_diagnostic_png(
    rgb: Any,
    output_path: Path,
    *,
    transform: RenderTransform,
    ...
    for row in range(rgb.shape[0]):
        for column in range(rgb.shape[1]):
            paint_lidar_diagnostic_cell(image, transform, metrics, column, row, rgb[row, column])
    draw_lidar_diagnostic_frame(image, metrics)
    pil_image = Image.fromarray(image, mode="RGBA")
    draw = ImageDraw.Draw(pil_image)
    title_font = pillow_layout_font(max(12, int(round(3.2 * px_per_mm))))
...
def render_lidar_contour_diagnostic_png(
...
def render_laserscan_diagnostic_pngs(source_data: dict[str, Any], output_base: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    surfaces = lidar_diagnostic_surfaces(source_data, transform)
    reports: dict[str, Any] = {}
    stem = output_base.name

    reports["contours_1m"] = render_lidar_contour_diagnostic_png(
        source_data,
    ...
    for prefix, values in (("ground", surfaces["ground_height"]), ("surface", surfaces["surface_height"])):
        ...
        reports[f"{prefix}_gradient"] = render_lidar_raster_diagnostic_png(
            gradient,
        ...
        reports[f"{prefix}_shading"] = render_lidar_raster_diagnostic_png(
        ...
        slope = slope_degrees(values)
        slope_rgb, slope_max = grayscale_rgb(slope, min_value=0.0, max_value=45.0, invert=True)
        reports[f"{prefix}_slope"] = render_lidar_raster_diagnostic_png(
            slope_rgb,
    ...
    reports["ground_coverage"] = render_lidar_raster_diagnostic_png(
        coverage_rgb,
    ...
    reports["minimum_object_height"] = render_lidar_raster_diagnostic_png(
        min_object_rgb,
    ...
    reports["point_count_up_to_5m"] = render_lidar_raster_diagnostic_png(
        point_count_rgb,
    ...
    reports["vegetation_height"] = render_lidar_raster_diagnostic_png(
        vegetation_rgb,
